...
and print out the search result from the high relative to the low relative.
...
#include<string.h>
using namespace std;
// This function is to print the content of txt file of the course which is the user wants to know the content.
...
// This function is to display the search result by relative scores' order (high to low) and offer some interactions.
void DisplayResult(float* scores){
    ...
    //This part is to sort the 81 scores from high to low by using Bubble sort.
    float t;
    for(int i=0;i<80;i++){
        int flag1=0;
    ...
    // This part is to clean the same scores and 0 scores(correlation is 0), only leave one in the single_scores array by order(from high to low).
    int count=0;// The count is used to count the number of items in the single_scores array.
    ...
    for(int i=0;i<81;i++){
        ...
        for(int j=0;j<count;j++){
            // if scores==0, it will also not be concluded in  single_scores which only stores relevant courses' scores.
        ...
        while(true) {
            do {
                ...
                cin >> input;
                     ...
                     (!((strlen(input) == 1) && (input[0] >= 48) && (input[0] <= 57))));// Input validation
            cout << endl;
