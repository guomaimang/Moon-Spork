...
#include <string.h>
using namespace std;
// This function is to print the content of txt file of the course which is the user wants to know.
...
void DisplayResult(int *scores)
{
    ...
    //This is to sort the 81 scores from high to low by using bubble sort.
    int t;
    for (int i = 0; i < 80; i++)
    {
        int flag1 = 0;
    ...
    // This part is to record the scores that occurred and only once is recorded for recurring scores.
    int count = 0;
    ...
    while (true)
    {
        ...
        do
        {
            do
            {
                cout << "Enter 'c' to show content, 'd' to download the subject description form, 'e' to choose another relevant course, 0 to finish : ";
                cin >> choice;
            } while (!((strlen(choice) == 1) && (choice[0] == 'c' || choice[0] == 'd' || choice[0] == 'e'||choice[0] == '0')));
            //Display the content
            if (choice[0] == 'c')
            {
